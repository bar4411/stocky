...
from data.schemas import LeadAgentReport, DiscoveryOutput, RiskLevel
from data.stocks_prefilter import Prefilter
...
class StocksRecommenderPipeline:
    ...
    def run_agents(self):
        ...
        all_stocks_lead_agents = self.agents_manager.analyze(self.stocks_data)
    ...
    def process_output(self):
        output = DiscoveryOutput(
            run_date=self.config.start_time,
            ...
            total_duration_seconds=(time.time() - self.config.start_time).total_seconds(),
        )
